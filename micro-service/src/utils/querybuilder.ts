...
class QueryBuilder {
  ...
  filter() {
    ...
    where = this.parseLogicalOperators(where);
  ...
  paginate() {
    ...
    perPage =
      parseInt(perPage) || Number(process.env.PAGINATION_PER_PAGE) || 15;
  ...
  build(actions?: ('filter' | 'sort' | 'select' | 'paginate')[]) {
  ...
  private normalizeMathOperatorsRecursive(filters: any) {
    ...
    for (const key in filters) {
      ...
      if (typeof value !== 'string' && !Array.isArray(value))
        ...
        normalized = { ...normalized, [key]: parseInt(value as any) || value };
      else normalized = { ...normalized, [key]: value };
    }

    console.log('normalizd ', normalized);

    return normalized;
  }

  private parseLogicalOperators(
    query: Record<
      string,
      | string
      | {
    ...
    for (const key in q) {
      ...
      if (
        typeof value === 'object' &&
        ('or' in value || 'and' in value || 'not' in value)
      ) {
          ...
          if (this.isLogicalOperator(operator)) {
            q = {
              ...
              [operator.toUpperCase()]: [
                ...(q[operator.toUpperCase()] || []),
                ...value[operator]
                  .split(',')
                  .map((v: string) => ({ [key]: v })),
              ],
            };
            delete q[key][operator];
          }
        }

        if (Object.keys(q[key]).length <= 0) delete q[key];
      }
    }
  ...
  private isLogicalOperator(operator: string): boolean {
    return ['or', 'and', 'not'].includes(operator.toLowerCase());
  }
